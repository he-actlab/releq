...
class RPolicy_net:
    def __init__(self, name: str, lstm_cell, temp=0.5, num_actions=3, num_layers=5):
        """
        :param name: string
        :param env: gym env
        :param temp: temperature of boltzmann distribution
        ...
        #act_space = env.action_space
        batch_size = 1
        self.rnn_initial_state_in = lstm_cell[0].zero_state(1, tf.float32) #Argument 1 is the batch_size
        #self.initial_cell_state = np.zeros((1, 5))
        #self.initial_hidden_state = np.zeros((1, 5))
        #self.rnn_initial_state_in = lstm_cell.LSTMStateTuple(self.initial_cell_state, self.initial_hidden_state)
        self.rnn_state_in = lstm_cell[0].zero_state(1, tf.float32)

        with tf.variable_scope(name):
            self.obs = tf.placeholder(dtype=tf.float32, shape=[None] + list([num_layers, 5]), name='obs')

            obs_rnn_input = tf.reshape(self.obs, [batch_size, num_layers, 5])
            self.rnn_1, self.rnn_state_1 = tf.nn.dynamic_rnn(inputs=obs_rnn_input, cell=lstm_cell[0], dtype=tf.float32, initial_state=self.rnn_state_in)
            ...
            with tf.variable_scope('policy_net'):
                ...
                #layer_3 = tf.layers.dense(inputs=layer_2, units=128, activation=tf.nn.relu, name = 'policy_layer_3')
                self.rnn_2, self.rnn_state_2 = tf.nn.dynamic_rnn(inputs=self.rnn_1, cell=lstm_cell[1], dtype=tf.float32, initial_state=self.rnn_state_in)
                self.rnn_2 = tf.reshape(self.rnn_2,shape=[-1, 128])

                self.act_probs = tf.layers.dense(inputs=self.rnn_2, units=num_actions, activation=tf.nn.softmax)
                #with tf.variable_scope('policy_layer_2', reuse=True):
                #    self.layer_2_weights = tf.get_variable('kernel')
                #with tf.variable_scope('policy_layer_3', reuse=True):
                #    self.layer_3_weights = tf.get_variable('kernel')
            ...
            with tf.variable_scope('value_net'):
                #layer_1 = tf.layers.dense(inputs=self.obs, units=40, activation=tf.nn.relu)
                self.rnn_vnet = tf.reshape(self.rnn_1,shape=[-1, 128])
                layer_2 = tf.layers.dense(inputs=self.rnn_vnet, units=128, activation=tf.nn.relu)
                layer_3 = tf.layers.dense(inputs=layer_2, units=64, activation=tf.nn.relu)
                self.v_preds = tf.layers.dense(inputs=layer_3, units=1, activation=None)
    ...
    def act(self, obs, stochastic=True):
        if stochastic:
            return tf.get_default_session().run([self.act_stochastic, self.v_preds, self.rnn_state_1], feed_dict={self.obs: obs})
        else:
            return tf.get_default_session().run([self.act_deterministic, self.v_preds, self.rnn_state_1], feed_dict={self.obs: obs})
...
class RLQuantization:
    def __init__(self, num_layers, accuracy, network_name, layer_names, layer_state_info):
        ...
        policy_lstm_cell = [tf.contrib.rnn.LSTMCell(num_units=128, state_is_tuple=True), tf.contrib.rnn.LSTMCell(num_units=128, state_is_tuple=True)] #tf.contrib.cudnn_rnn.CudnnLSTM(1, num_units=4, kernel_initializer=tf.contrib.layers.xavier_initializer())
        old_policy_lstm_cell = [tf.contrib.rnn.LSTMCell(num_units=128, state_is_tuple=True), tf.contrib.rnn.LSTMCell(num_units=128, state_is_tuple=True)] #tf.contrib.cudnn_rnn.CudnnLSTM(1, num_units=4, kernel_initializer=tf.contrib.layers.xavier_initializer())

        """ Load the agent """
        self.Policy = RPolicy_net('policy', policy_lstm_cell, num_actions=len(self.supported_bit_widths), num_layers=self.num_layers)
        self.Old_Policy = RPolicy_net('old_policy', old_policy_lstm_cell, num_actions=len(self.supported_bit_widths), num_layers=self.num_layers)
        ...
        self.yaml_file = "resnet_bn_dorefa.yaml"
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for i in range(self.total_episodes):
                ...
                for layer_num in range(self.num_layers):
                    intial_layer_state = [self.layer_state_info.loc[layer_num, 'layer_idx_norm'], self.layer_state_info.loc[layer_num, 'n'], self.layer_state_info.loc[layer_num, 'c'], self.layer_state_info.loc[layer_num, 'k'], self.layer_state_info.loc[layer_num, 'std']]
                ...
                #act_index: 0-> Dec Bits, 1-> Keep Same, 2->Inc Bits
                act_indexes, v_pred_arr, self.Policy.rnn_state_in_new = self.Policy.act(obs=[s_history], stochastic=True)
                action_probabilities = self.Policy.get_action_prob(obs=[s_history])
                for i in range(self.num_layers):
                    print("Action probabilities for layer ", i, " are ", action_probabilities[i])
                #l2w, l3w = self.Policy.get_policy_weights()
                #if np.isnan(l1w).any():
                #    print("L1W ", l1w)
                #    exit()
                ...
                for j in range(self.num_layers):
                    act_index = act_indexes[j]
                    a_history.append(act_index)
                    new_bitwidth = self.perform_flexible_action(act_index)
                    bitwidth_layers[j] = new_bitwidth
                    v_pred = v_pred_arr[j][0]
                    v_preds.append(v_pred)
                ...
                for j in range(self.num_layers):
                    if j == self.num_layers-1:
                        os.system("python3 compress_classifier.py --arch resnet20_cifar ../../../data.cifar --epochs 10 --lr 0.001 --resume resnet.pth.tar --compress resnet_bn_dorefa.yaml")
                        cur_accuracy = float(open("val_accuracy.txt").readlines()[0])
                    ...
                    data = [i, j, self.quant_state, cur_accuracy, reward]
                    data2 = action_probabilities[j]
                ...
                print(rewards, v_preds)
                gaes = self.PPO.get_gaes(rewards=rewards, v_preds=v_preds, v_preds_next=v_preds_next)

                # convert list to numpy array for feeding tf.placeholder
                observations = np.reshape(s_history, newshape=[-1] + list([5]))
                actions = np.array(a_history).astype(dtype=np.int32)
                rewards = np.array(rewards).astype(dtype=np.float32)
                v_preds_next = np.array(v_preds_next).astype(dtype=np.float32)
                gaes = np.array(gaes).astype(dtype=np.float32)
                ...
                for epoch in range(1):
                    ...
                    sampled_inp = [np.take(a=a, indices=sample_indices, axis=0) for a in inp]  # sample training data
                    self.PPO.train(obs=np.reshape(sampled_inp[0], newshape=[1, 20, 5]), actions=sampled_inp[1], rewards=sampled_inp[2], v_preds_next=sampled_inp[3], gaes=sampled_inp[4])
                
                summary = self.PPO.get_summary(obs=np.reshape(sampled_inp[0], newshape=[1, 20, 5]), actions=inp[1], rewards=inp[2], v_preds_next=inp[3], gaes=inp[4])[0]
...
def write_to_csv(step_data):
    with open('resnet_learning_history_log.csv', 'a') as csvFile:
...
with open('releq_resnet_learning_history_log.csv', 'w') as writeFile:
...
#layer_names = ["model.0.0", "model.1.0", "model.1.3", "model.2.0", "model.2.3","model.3.0","model.3.3","model.4.0","model.4.3","model.5.0","model.5.3","model.6.0","model.6.3","model.7.0","model.7.3","model.8.0","model.8.3","model.9.0","model.9.3","model.10.0","model.10.3","model.11.0", "model.11.3", "model.12.0", "model.12.3", "model.13.0", "model.13.3", "model.fc"]
...
rl_quant = RLQuantization(number_of_layers, 93, network_name, layer_names, layer_state_info) #num_layers, accuracy, network_name, layer_names, layer_stats
