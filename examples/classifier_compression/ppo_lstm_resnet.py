...
class RPolicy_net:
    def __init__(self, name: str, lstm_cell, temp=0.5, num_actions=3):
        ...
        with tf.variable_scope(name):
            self.obs = tf.placeholder(dtype=tf.float32, shape=[None] + list([20, 5]), name='obs')

            obs_rnn_input = tf.reshape(self.obs, [batch_size, 20, 5])
...
rl_quant.quantize_layers_together(20)
