...
class RLQuantization:
    def __init__(self, num_layers, accuracy, network_name, layer_names, layer_state_info, cmd, yaml_file, quant_type):
    ...
    def update_yaml_file(self, weight_bitwidth_layers):
        ...
        for i, layer_name in enumerate(self.layer_names):
            ...
            self.yaml_out["quantizers"][self.quant_type]["bits_overrides"][layer_name]["wts"] = weight_bitwidth_layers[i]
    ...
    def calculate_reward_shaping(self, cur_accuracy):
        ...
        ny = np.linspace(0.0, ylim, levels)

        #y, x = np.meshgrid(nx, ny)

        z = np.zeros((len(nx), len(ny)))
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            ...
            for i in range(self.total_episodes):
                ...
                for layer_num in range(self.num_layers):
                    ...
                    data = [i, layer_num, self.quant_state, cur_accuracy/self.fp_accuracy, reward]
                    data2 = self.Policy.get_action_prob(obs=[s])
...
rl_quant = RLQuantization(number_of_layers, 83, network_name, layer_names, layer_state_info, training_cmd, yaml_file, quant_type) #num_layers, accuracy, network_name, layer_names, layer_stats
